...
const LoginPage = () => {
  ...
  return (
    ...
    <h2 className='font-medium text-2xl flex justify-between items-center'>{currState}
      ...
      {
        currState === 'Sign Up' && isDataSubmitted && (
          <textarea rows = {4} required onChange={(e) => setBio(e.target.value)} value={bio} placeholder='provide a short bio' className='p-2 border border-gray-500 rounded-md focus:outline-none focus:ring-2 focus:ring-indigo-500'></textarea>
        )
      }
      ...
      <div className='flex flex-col gap-2'>
        {
          currState === 'Sign Up'
          ? <p className='text-sm text-gray-600'>Already have an account? <span onClick={() => {setCurrState('Login'); setIsDataSubmitted(false)}} className='text-blue-500 cursor-pointer'>Login</span></p>
          : <p className='text-sm text-gray-600'>Don't have an account? <span onClick={() => setCurrState('Sign Up')} className='text-blue-500 cursor-pointer'>Sign Up</span></p>
        }
