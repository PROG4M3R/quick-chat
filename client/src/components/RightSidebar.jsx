...
const RightSidebar = ({ showRightSidebar }) => {
  ...
  return selectedUser && showRightSidebar && (
    <div className= {`bg-[#8185B2]/10 text-white w-full relative overflow:y-scroll ${
      ...
      <button onClick={() => logout()} className='absolute bottom-5 left-1/2 transform -translate-x-1/2 bg-linear-to-r from-purple-400 to-violet-600 text-white border-none text-sm foont-light py-2 px-20 rounded-full cursor-pointer'>
        Logout
      </button>
    </div>
  )
